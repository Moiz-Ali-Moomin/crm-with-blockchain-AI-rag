...
function CurrencyDropdown({ value, onChange }: { value: Currency; onChange: (c: Currency) => void }) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg z-10 min-w-[130px]">
          {(['INR', 'USD'] as Currency[]).map((c) => (
            <button
              key={c}
              onClick={() => { onChange(c); setOpen(false); }}
              className={cn(
                'w-full text-left px-4 py-2.5 text-sm hover:bg-gray-50 transition-colors first:rounded-t-xl last:rounded-b-xl',
                value === c ? 'text-blue-600 font-semibold' : 'text-gray-700',
              )}
            >
              {c === 'INR' ? '🇮🇳 INR — ₹' : '🇺🇸 USD — $'}
            </button>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white border border-gray-200 rounded-xl w-full max-w-md shadow-xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <p className="text-xs text-gray-400 uppercase tracking-widest mb-0.5">Upgrading to</p>
            <h3 className="text-base font-semibold text-gray-900">{plan.name} — {displayPrice}/mo</h3>
          </div>
          ...
          {currency === 'INR' ? (
            <>
              ...
              <button
                ...
                className="w-full flex items-center gap-4 p-4 rounded-xl border border-gray-200 hover:border-blue-400 hover:shadow-sm transition-all group disabled:opacity-50 disabled:cursor-not-allowed"
              >
                ...
                <div className="text-left flex-1">
                  <p className="text-sm font-medium text-gray-900">UPI AutoPay / Card Mandate</p>
                  <p className="text-xs text-gray-400">Recurring via Razorpay · UPI, Visa, RuPay</p>
                </div>
                {loading === 'razorpay'
              ...
              <button
                ...
                className="w-full flex items-center gap-4 p-4 rounded-xl border border-gray-200 hover:border-blue-400 hover:shadow-sm transition-all group disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <div className="w-10 h-10 rounded-xl bg-gray-50 border border-gray-200 flex items-center justify-center flex-shrink-0 group-hover:bg-blue-50">
                  <CreditCard size={18} className="text-blue-600" />
                </div>
                <div className="text-left flex-1">
                  <p className="text-sm font-medium text-gray-900">Pay Once (Card / UPI / Netbanking)</p>
                  <p className="text-xs text-gray-400">One-time via Razorpay · no mandate</p>
                </div>
            ...
            <>
              ...
              >
                ...
                <div className="text-left flex-1">
                  ...
                  <p className="text-xs text-gray-400">Powered by Stripe · Visa, Mastercard, Amex</p>
                </div>
                {loading === 'stripe'
                  ? <div className="w-4 h-4 border-2 border-blue-500 border-t-transparent rounded-full animate-spin" />
                  : <ChevronRight size={16} className="text-gray-300 group-hover:text-blue-500" />}
              ...
              >
                ...
                <div className="text-left flex-1">
                  ...
                  <p className="text-xs text-gray-400">Pay with your PayPal balance or linked bank</p>
                </div>
                {loading === 'paypal'
                  ? <div className="w-4 h-4 border-2 border-blue-500 border-t-transparent rounded-full animate-spin" />
                  : <ChevronRight size={16} className="text-gray-300 group-hover:text-blue-500" />}
...
function CurrentPlanCard({ info, onCancel, cancelling }: {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6">
      <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-gray-400 mb-4">Current Plan</p>
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 justify-between">
        <div className="space-y-1.5">
          <div className="flex items-center gap-2.5">
            <Zap size={16} className="text-blue-600" />
            <span className="text-lg font-semibold text-gray-900 capitalize">{info.plan}</span>
  ...
  return (
    <div className={cn(
      ...
      isCurrent
        ...
        : isHighlighted
          ? 'border-blue-300 bg-white hover:border-blue-400 hover:shadow-sm'
      ...
      {isCurrent && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2">
          <span className="bg-white text-blue-600 text-xs font-semibold px-3 py-0.5 rounded-full border border-blue-300">
            Current Plan
          </span>
        </div>
      )}
      ...
      <div>
        <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-gray-400 mb-1">{plan.name}</p>
        <div className="flex items-baseline gap-1">
          {plan.price === 0 ? (
            <span className="text-2xl font-bold text-gray-900">Free</span>
          ) : currency === 'INR' && inrPrice ? (
            <>
              <span className="text-2xl font-bold text-gray-900">₹{inrPrice.toLocaleString('en-IN')}</span>
      ...
      <Button
        ...
        className={cn(
          ...
          isCurrent
            ...
            : plan.id === 'free'
              ? 'bg-gray-100 text-gray-400 border border-gray-200 cursor-default hover:bg-gray-100'
    ...
    <div className="flex items-center justify-between py-3 border-b border-gray-100 last:border-0">
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          <p className="text-sm text-gray-900 truncate">{invoice.number ?? invoice.id}</p>
          <p className="text-xs text-gray-400">{fmtDate(invoice.created)}</p>
        </div>
      </div>
    ...
    <>
      ...
      <div className="space-y-8 max-w-4xl">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-base font-semibold text-gray-900">Billing & Subscription</h1>
            <p className="text-sm text-gray-500 mt-0.5">Manage your plan, payment method, and invoices.</p>
          </div>
        ...
        {/* Plans */}
        <div>
          <h2 className="text-[11px] font-semibold uppercase tracking-[0.12em] text-gray-400 mb-4">Plans</h2>
          {plansLoading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="h-64 bg-gray-100 rounded-xl border border-gray-200 animate-pulse" />
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {billingInfo.stripeSubscriptionId && (
          <div>
            <h2 className="text-[11px] font-semibold uppercase tracking-[0.12em] text-gray-400 mb-4">Invoice History</h2>
